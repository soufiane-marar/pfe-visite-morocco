...
@Component({
  selector: 'app-hebergement-dialog',
  templateUrl: './hebergement-dialog.component.html',
  styleUrls: ['./hebergement-dialog.component.scss']
})
export class HebergementDialogComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder,
              ...
              public dialogRef: MatDialogRef<HebergementDialogComponent>,
              @Inject(MAT_DIALOG_DATA) public data: any) {
  }

  ngOnInit(): void {

    let hbg: any = this.data.hebergement;
    ...
    this.formGrp = this.formBuilder.group(
      {
        ...
        ranking: new FormControl(hbg ? hbg.extra.ranking : 1, [Validators.required]),
        rooms: new FormControl(hbg ? hbg.extra.rooms : 1, [Validators.required]),
        wifi: new FormControl(hbg ? hbg.extra.wifi : false),
        restaurant: new FormControl(hbg ? hbg.extra.restaurant : false),
        piscine: new FormControl(hbg ? hbg.extra.piscine : false),
  ...
  public confirm(): void {
    ...
    if (this.media.length == 0) {
      this.alertBoxService.alert({icon: 'warning', html: `<p>Veuillez ajouter ou moins une image avec les critères suivantes : </p>
...
Taille : ${environment.imageValidation.size / 1000}Ko</p>`});
  ...
  public selectLogo(event: any): void {
    ...
    this.checkImageDimensions(event.target.files[0], 512, 512, 90000)
      .then(() => {
  ...
  private add(): void {
    ...
    this.hebergementsService.addHebergement(body)
      .pipe(take(1))
      .subscribe(
        value => {
  ...
  private edit(): void {
    ...
    this.ngxSpinner.show();
    this.hebergementsService.editHebergement(body, this.data.hebergement.reference)
      .pipe(take(1))
      .subscribe(
        value => {
          this.ngxSpinner.hide();
